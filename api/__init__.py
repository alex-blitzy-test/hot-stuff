...
api/routes.py add the JSON endpoints under /api/*. (Source: api/__init__.py:L8)

CORS: CORS(app) enables cross-origin requests so clients served from another
origin may call the /api/* endpoints. (Source: api/__init__.py:L9)
...
(Source: docker-compose.yml), not a literal DNS hostname; Compose's internal
...
app back from this module. (Source: api/__init__.py:L21)
