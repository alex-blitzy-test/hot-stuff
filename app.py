...
api/__init__.py:L64) and, when the module is executed directly
...
api/__init__.py:L54-L87).

Running the server:
    * Container (default): the image's ``CMD ["python3", "app.py"]`` runs this
      file (Source: Dockerfile:L15), which launches the **Flask development
    ...
    * Docker Compose: ``docker-compose up`` builds and starts this service
      together with PostgreSQL (Source: docker-compose.yml:L3-L22).
    * Local development: ``flask run`` using the settings in ``.flaskenv``
      (``FLASK_APP=app.py`` and ``FLASK_ENV=development``;
      Source: .flaskenv:L1-L2), or run this module directly with
      ``python3 app.py``.
...
if __name__ == '__main__':
    ...
    # (Source: Dockerfile:L13) and Compose maps host port 80 to container port
    # 5000 (Source: docker-compose.yml:L10-L11). No port is passed here,
    # so Flask falls back to its default port 5000.
